...
//				reqMetric);
		...
		assertTrue(metric.numberOfEvents() == 3);
		assertTrue(metric.duration() == 2.25);
		
		for (Metric subMetric : metric) {
			assertTrue(subMetric.numberOfEvents() == 3);
			assertTrue(subMetric.duration() == 0.75);
			for (Metric subSubMetric : subMetric) {
				assertTrue( subSubMetric.numberOfEvents() == 1);
				assertTrue( subSubMetric.duration() == 0.25);
			}
		}
	}
...
//				reqMetric);
		...
		assertTrue(metric.numberOfEvents() == 2);
		assertTrue(metric.duration() == 1);
		for (Metric subMetric : metric) {
			assertTrue(subMetric.numberOfEvents() == 2);
			assertTrue(subMetric.duration() == 0.5);
			for (Metric subSubMetric : subMetric) {
				assertTrue(subSubMetric.numberOfEvents() == 1);
				assertTrue(subSubMetric.duration() == 0.25);
			}
...
//				reqMetric);
	...
	public void testMetric_3_1_4() {
		...
		assertTrue(metric1.numberOfEvents() == 3);
		assertTrue(metric1.duration() == 0.75);
		assertTrue(metric2.numberOfEvents() == 1);
		assertTrue(metric2.duration() == 0.25);
...
//				reqMetric);
		...
		assertTrue(metric.numberOfEvents() == 2);
		assertTrue(metric.duration() == 1.25);
		Metric metric1 = metric.get(0);
		assertTrue(metric1.numberOfEvents() == 3);
		assertTrue(metric1.duration() == 1.125);
		for (Metric subMetric : metric1) {
			assertTrue(subMetric.numberOfEvents() == 2);
			assertTrue(subMetric.duration() == 0.375);
			for (Metric subSubMetric : subMetric) {
				assertTrue(subSubMetric.numberOfEvents() == 3);
				assertTrue(subSubMetric.duration() == 0.1875);
				for (Metric subSubSubMetric : subSubMetric) {
					assertTrue(subSubSubMetric.numberOfEvents() == 1);
					assertTrue(subSubSubMetric.duration() == 0.0625);
				}
			}
		}
		...
		Metric metric2 = metric.get(1);
		assertTrue(metric2.numberOfEvents() == 1);
		assertTrue(metric2.duration() == 0.125);
	}
...
//		Testcase testcase = new Testcase("Test the validity of metrics regarding the pattern: " + Metric.METRIC_PATTERN, reqMetric);
		assertTrue(MetricTools.isValidMetric("3/4"));
		assertTrue(MetricTools.isValidMetric(" 3 / 4 "));
		assertTrue(MetricTools.isValidMetric("3+2+8/4"));
		assertTrue(MetricTools.isValidMetric(" 3 + 2 + 8 / 4 "));
		assertTrue(MetricTools.isValidMetric(" 3 / 4 - 8 / 8 "));
	}
	
	@Test
	public void testInvalidMetric() {
		// Testcase testcase = new Testcase("Test invalid merics regarding the pattern given.", reqMetric);
		assertFalse(MetricTools.isValidMetric(""));
		assertFalse(MetricTools.isValidMetric("3"));
		assertFalse(MetricTools.isValidMetric("a/4"));
		assertFalse(MetricTools.isValidMetric(" +3 / +4"));
		assertFalse(MetricTools.isValidMetric(" +3 / 4"));
		assertFalse(MetricTools.isValidMetric(" 3 / +4"));
	}
	...
	public void testDepth() {
		...
		for (Metric subMetric : metric) {
			...
			for (Metric subSubMetric : subMetric) {
				...
				assertTrue(subSubMetric.depth() == 2);
				assertTrue(subSubMetric.numberOfEvents() == 2);
				for (Metric subSubSubMetric : subSubMetric) {
					// System.out.println(" " + subSubSubMetric + ": depth=" + subSubSubMetric.depth());
					assertTrue(subSubSubMetric.depth() == 1);
					assertTrue(subSubSubMetric.numberOfEvents() == 3);
				}
			}
		}
		assertTrue(depths[0] == 3);
		assertTrue(depths[1] == 0);
	}
	...
	public void test54() {
		...
		assertTrue(metric.depth() == 2);
		assertTrue(metric.numberOfEvents() == 2);
		assertTrue(metric.duration() == 1.25);
		Metric subMetric1 = metric.get(0);
		Metric subMetric2 = metric.get(1);
		assertTrue(subMetric1.numberOfEvents() == 3);
		assertTrue(subMetric1.duration() == 0.75);
